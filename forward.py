...
def forward(inputs, y1, y2, y3, y4, alpha1, alpha2, alpha3, istrain):
    ...
    #条件归一化
    inputs = conditional_normalization(inputs, "cin1", y1, y2, y3, y4, alpha1, alpha2, alpha3, istrain)
    #relu
...
及论文：  He, Kaiming, et al. “Deep Residual Learning for Image Recognition.”  arXiv:1512.03385 (2015).
...
def vggnet(inputs, vgg_path):
    ...
    F = {}  # 定义返回值为字典F
    inputs = tf.nn.relu(vgg_conv(inputs, para["conv1_1"][0], para["conv1_1"][1]))  # 使用vgg中的conv1_1,conv1_2完成卷积
    inputs = tf.nn.relu(vgg_conv(inputs, para["conv1_2"][0], para["conv1_2"][1]))  # 激活函数 relu，即将矩阵中每行的非最大值置0。
    F["conv1_2"] = inputs
    inputs = vgg_max_pooling(inputs)  # 池化
    inputs = tf.nn.relu(vgg_conv(inputs, para["conv2_1"][0], para["conv2_1"][1]))  # 第二层卷积提取特征
    inputs = tf.nn.relu(vgg_conv(inputs, para["conv2_2"][0], para["conv2_2"][1]))
    F["conv2_2"] = inputs
    inputs = vgg_max_pooling(inputs)  # 池化
    inputs = tf.nn.relu(vgg_conv(inputs, para["conv3_1"][0], para["conv3_1"][1]))  # 第三层卷积提取特征
    inputs = tf.nn.relu(vgg_conv(inputs, para["conv3_2"][0], para["conv3_2"][1]))
    inputs = tf.nn.relu(vgg_conv(inputs, para["conv3_3"][0], para["conv3_3"][1]))
    F["conv3_3"] = inputs
    inputs = vgg_max_pooling(inputs)  # 池化
    inputs = tf.nn.relu(vgg_conv(inputs, para["conv4_1"][0], para["conv4_1"][1]))  # 第四层卷积提取特征
    inputs = tf.nn.relu(vgg_conv(inputs, para["conv4_2"][0], para["conv4_2"][1]))
    inputs = tf.nn.relu(vgg_conv(inputs, para["conv4_3"][0], para["conv4_3"][1]))
    F["conv4_3"] = inputs
    return F
